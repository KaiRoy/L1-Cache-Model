...
//		}
	...
	// Calc and print Cache Hit Ratio
	float chr = (float)chit/(float)(chit+cmiss);
...
int instrFetch(struct address addr) {
	...
	enum MESI_STATE state;
	way = LRU_Instr(addr);
	state = MESI_Protocol(addr, way, 'i', 0, 1, 0, 0);
	if (miss) {
		if (mode)
...
int dataReqL2(struct address addr) {
	...
	if (way != -1) {
		state = MESI_Protocol(addr, way, 'd', 1, 0, 0, 1);
		if (mode) {
...
//Start of INVALID transitions
    if 		 (snoopBit && invalidateData) {
    	state = I;
	}else if ((state == I) && (snoopBit == 1) && (readingData == 1)){
        state = S;
        if (mode && wb) {
			printf("Write to L2 %x (Write Back)\n", (1048576*backup.tag + 64*backup.index));
			wb = 0;
        }
